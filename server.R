...
function(input, output) {
    ...
    output$dygraph <- renderDygraph({
        ...
        if (input$showbars) {
            ...
            name = c("low_price", "weighted_value", "high_price")
        }
        else {
            xts <- xts(cbind(data$weighted_value, data$sum_amount), order.by = data$date, frequency = 1)
            names(xts) <-  c("weighted_value", "sum_amount")
            name <- "weighted_value"
        }
        
        # Plot with htmlwidget dygraph
        dygraph(xts, ylab= input$currency, main = paste("Market:", input$market)) %>% 
            dySeries(name = name, label="Weighted Value", color = "Blue") %>%  
